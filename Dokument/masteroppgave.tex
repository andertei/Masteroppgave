...
It is common to distinguish an arc into too two different types of arc. The one an most common is called a dynamic arc, the other one is called a static arc. A static arc can only be established when using DC current after any transients have died out. Therefore static arcs are uncommon and in nearly all switching operations the properties of the arc is best described to be of an dynamic art.

AC currents always generates a dynamic arc, this is because the current alters with time, and thereby the properties of the arc. In cases where DC current is used, but externals factors like the cooling from a puffer varies by time the arc is defined as a dynamic arc. When dealing with AC switchgear with a puffer based interruption method the arc is always a dynamic arc. Due to the complexity of a dynamic arc it is common to regard the arc to behave like a static arc within a certain time interval, and therefore some of the static arc properties will be described in this section. Dynamic arcs will be featured further in section \ref{sec:dynARC}.
...
In both figure \ref{fig:threeTests} and \ref{fig:averageArcingVoltage} contact separation occurs at $t \approx 0.0140$ s, the first CZ at $t \approx 0.0235$ and the second CZ at $t \approx 0.0335$ s. The arcing time from contact separation to the first CZ is about the same for each test approximately 9.5 ms.

When the length of the arc is short most of the voltage will be across the electrodes. The length of the arc is determinated by the distance between the contacts, which moves apart with a constant speed of approximately 5 m$/$s. To establish an estimate over the voltage across the electrodes the the average voltage when the contacts was between 0.4 mm and 1.4 mm apart was calculated. Due to distortion of the arcing voltage when the arc ignites, contact gap length less than 0.4 mm was unsuited for this use. Due to the length of the arc, the average arcing voltage can therefore represent an approximation of the voltage across the electrodes, and it was calculated too 33 volt in absolute value.
