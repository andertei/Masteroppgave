...
In figure \ref{fig:arcingVoltageDrop} the arcing voltage for a interruption test for geometry \textbf{b} at 880 A is presented. The arc had a thermal re-ignition in all three CZ shown in the figure and a upstream over-pressure of 0.25 bar was used. The red square in figure \ref{fig:arcingVoltageDrop} shows a part of the graph where a sudden drop in the arcing voltage occurred. The area is magnified in figure \ref{fig:arcingVoltageDrop_zoomed}. Both figures have been processed in matlab to filter out measuring noise. The filter used to cancel noise was tuned so that it did not impact the sudden drop in arcing voltage shown in the figures. It is not uncommon for sudden drops in the arcing voltage, like the one presented here, to occur after a CZ and a possible explanation for these drops are going to be discussed in section \ref{sec:disc_drop_arcing_voltage} of this report.
