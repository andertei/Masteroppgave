...
A gas with a low time constant is quicker to cool and thereby reduces its electric conductivity faster than an gas with a high time constant. This is an advantage during current interruption. Air has a fairly high time constant resulting inn a slower cool-down time than other interrupting gases. This is closely related to the problems mentioned in section \ref{sec:HeatTransport}. In figure \ref{fig:arcingVoltageFre} the arcing voltage is shown for a dynamic arc, as illustrated by the figure it varies both with current and frequency.
...
\caption{Arcing voltage during a power cycle for test switch FINN PAA EN BEDRE TITTEL.} \label{fig:arcingVoltageVSCurrent}
\end{figure}
