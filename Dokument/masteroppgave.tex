...
If regarding the 630 A test for geometry \textbf{a} and \textbf{b} the interruption result is almost equal for the two geometries. Due to results obtained through previously conducted experiments it was expected that both geometries performed well for the 630 A test, and interrupted the current with a low upstream over-pressure, which they both did. This is because these studies have shown that a short narrow nozzle is preferred opposed to a long and wide one.

As explained in section \ref{sec:testSwitchandContact} the nozzle for geometry \textbf{a} and \textbf{b} consists of two parts: One cylindrical part, with almost the same diameter as the contact pin, and one part shaped as a funnel at the end of the nozzle. Both parts designed to obtain certain preferable air flow features during interruption.

The effect of the cylindrical part of the nozzle became clear during testing due to a drop in the interruption success rate when the nozzle where starting to show signs of wear. For both geometries the cylindrical part was the first to deteriorate due to usage, since it became wider. Therefore, it is assumed that the tight cylindrical nozzle section had and impact on the interruption rate.  
