...
In figure \ref{fig:tempDist2}, shows how the temperature distribution varies with the electrical current. Due to radiation losses in the arc, the temperature has a upper limit of about 20 000 K to 30 000 K. At this point the cross-section of the arc will increase rather than the temperature. However, it is not common for an LBS to experience these temperature ranges, and its temperature distribution will mainly be in the lower current ranges in figure \ref{fig:tempDist2}.
...
It is common to distinguish an arc into two different types of arc. The one an most common is called a dynamic arc, the other one is called a static arc. A static arc can only be established when using DC current after any transients have died out. Therefore static arcs are uncommon and in nearly all switching operations the properties of the arc is best described by the dynamic arc model.

AC currents always generates a dynamic arc, this is because the current alters with time, and thereby the properties of the arc. In cases where DC current is used, but externals factors like the cooling varies by time like from a puffer, the arc is dynamic. When dealing with AC switchgear with a puffer based interruption method the arc is always a dynamic arc. Due to the complexity of a dynamic arc it is common to regard the arc to behave like a static arc within a certain time interval, and therefore some properties of a static arc will be described in this section. Dynamic arcs will be featured further in section \ref{sec:dynARC}.

The static arc characteristic if shown in figure \ref{fig:staticArcChar}. This describes the relationship between current and arcing voltage in an static arc. The scaling of the axes is only approximate and may vary with gas type and electrode material.
...
As can be seen from the figure the characteristic is highly non-linear. At low currents, like tens of amperes, the voltage drop across the arc decreases with increasing current. Then the voltage is constant, and apparently independent from the current flowing in the arc. How large, and in which current range the constant part of the arcing voltage occurs depends highly on what gas the arc burns in. For currents above this range the arcing voltage starts to increase with the current.

In figure \ref{fig:potDisArc} the potential distribution of an electrical arc is illustrated. An static electrical arc might be regarded as divided into three regions:
...
The description above is highly general and the voltage distribution will vary between which gas the arc is burning in, as well as the current range and which electrode materials that are used. Therefore the potential distribution across a dynamic arc, that burns in air for a typical current range for a LBS is analysed. The analyse are based on results from previously conducted test performed with the test circuit presented in section \ref{sec:testCir}.

First three interruption test where a thermal re-ignition occurred after the first CZ and the second CZ where chosen. The pressure in the pressure chamber was held constant throughout the whole test, and was set to 1.0 bar upstream pressure. The RMS value of the current was 630 A, while the contact and nozzle geometry was the same for each three tests. In figure \ref{fig:threeTests} the arcing voltage during the three chosen interruption tests are shown.
...
Then an average arcing voltage from these three test was calculated and presented in figure \ref{fig:averageArcingVoltage}. This average arcing voltage was then used to approximate the potential distribution of the burning arc.
...
In both figure \ref{fig:threeTests} and \ref{fig:averageArcingVoltage} contact separation occurs at $t \approx 0.0140$ s, the first CZ at $t \approx 0.0235$ and the second CZ at $t \approx 0.0335$ s. The arcing time, the time from the contact separation to the first CZ, was approximatly 9.5 ms for all the three tests.

The length of the arc is determinated by the distance between the contacts, which moves apart with a constant speed of approximately 5 m$/$s. When the length of the arc is short most of the voltage will be across the electrodes. To establish an estimate over the voltage across the electrodes the the average voltage when the contacts was between 0.4 mm and 1.4 mm apart was calculated. Due to transients in the arcing voltage when the arc ignites, contact gap length less than 0.4 mm was unsuited for this use. Due to the length of the arc, the average arcing voltage can therefore represent an approximation of the voltage across the electrodes, and it was calculated too 33 volt in absolute value.

Then the increase in arcing voltage per length is calculated. Linear regression when the arc is between 23.2 mm and 38.8 mm is used to establish the rate of raise in the arcing voltage, and thereby the increase in voltage per length can be established. This rate of raise was calculated to be 2.7 V$/$mm in absolute value.
...
This approximation of the potential distribution across the arc fits well in the region between contact separation and the first CZ. In the region between first and second CZ this model is not suited. It is assumed that the values calculated here will be dependent on both upstream pressure and test current.


\subsubsection{Dynamic arcing voltage} \label{sec:dynARC}
When the current, or cooling of the arc changes, the arcing voltage no longer follows the static arc characteristics presented in figure \ref{fig:staticArcChar}. This is because the temperature cannot change instantaneously, since it is impossible to use an infinite amount of power to heat or cool a certain mass instantaneously. This gives an arc a certain thermal inertia. During a short time span cooling can be regarded as constant, while the current is set by the input power.

Thermal inertia causes the arc to "remember" for a short time the amplitude of the current that just has passed. If the current follows a step function, the arc voltage will at first take a higher value, and then gradually decrease to the value corresponding to the static arc characteristic, as presented in figure \ref{fig:timeConstantStep}. This is because the arcing voltage is mainly set by the resistant in the arc, which is highly dependent on the temperature. Since the temperature cannot change instantaneously, the arcing voltage will be a function of time with approximately an exponential decay. The time constant of this decay varies from gas to gas. In table \ref{tab:timeConstants} the time constant for some gases is displayed. The time constant for each gas depends on several factors like test method and current magnitude. The variation of the time constant with regard to the current can to a certain degree be compared to the inverse of the thermal conductivity with regard to temperature presented in figure \ref{fig:tempConGas}. 
...
A gas with a low time constant is faster to cool and thereby reduces its electric conductivity faster than an gas with a high time constant. This is an advantage during current interruption. Air has a fairly high time constant resulting in a slower cool-down time than other interrupting gases. This is closely related to the problems mentioned in section \ref{sec:HeatTransport}. In figure \ref{fig:arcingVoltageFre} the arcing voltage for a dynamic arc is illustrated, as shown by the figure the arcing voltage varies both with current and frequency. In figure \ref{fig:arcingVoltageVSCurrent} the arcing voltage with regard to current for a previously conducted interruption test is shown. As can be observed from this figure, the arcing voltage follows the same pattern as an dynamic arc burning at 50 Hz shown in figure \ref{fig:arcingVoltageFre}. However, some differences are present since the arc in the interruption test is both cooled with a upstream pressure of 1.0 bar as-well as elongated by the separation of the contacts. 
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.8]{Bilder/Theory/arcingVoltagevzCurrent2.png}
\caption{Arcing voltage during a full power cycle in a test interruption.} \label{fig:arcingVoltageVSCurrent}
\end{figure}

The numbered parts in figure \ref{fig:arcingVoltageVSCurrent} represents different aspects of the interruption sequence, and are described in the list below. The arrows represents the direction of time during the interruption, a full power cycle is plotted in the figure resulting in a time span of 0.02 seconds.
\begin{description}
\item[1:] The contacts separates and the arc ignites. As the absolute value of the current increases the arcing voltage follows the same trend as illustrated in figure \ref{fig:arcingVoltageFre}.  
...
\item[7:] Mainly due to elongation of the arc the arcing voltage is higher in this point than in the start (point 1). If the length of the arc where held constant through out the whole experiment the arcing voltage would be expected to be approximately the same at point 6 and point 1. The arc have now burnt a full power cycle, t=0.02 s, from ignition.
\end{description}
\cleardoublepage
