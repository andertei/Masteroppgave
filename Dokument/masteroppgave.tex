...
In table \ref{tab:testParameters}, the value of the different test circuit parameters and the corresponding current can be observed. The test is done at currents with an RMS value of 630 A and 880 A. In the entire test the TRV is kept constant up to and including the first voltage peak. In the case of a failed interruption, thermal re-ignition occurs within a few microseconds after CZ.
...
\item $A_\mathrm{{funnel}}$ is defined as the area at the end of the nozzle, when x=20 mm, where the diameter of the funnel is at its biggest. This area is described by equation \eqref{eq:A_funnel}.
