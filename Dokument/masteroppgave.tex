...
It is common to distinguish an arc into two different types of arc. The most common is called a dynamic arc, the other one is called a static arc. A static arc can only be established when using DC current, and after any transients have died out. Therefore static arcs are uncommon and in nearly all switching operations the properties of the arc is best described by the dynamic arc model.

AC currents always generate a dynamic arc, this is because the current alters with time, and thereby the properties of the arc. In cases where DC current is used, but external factors like cooling varies by time, like in a puffer based switchgear the arc is dynamic. When dealing with AC switchgear with a puffer based interruption method the arc is always a dynamic arc. Due to the complexity of a dynamic arc, it is common to regard the arc to behave like a static arc within a certain time interval, and therefore some properties of a static arc will be described in this section. Dynamic arcs will be featured further in section \ref{sec:dynARC}.
...
The description above is highly general and the voltage distribution will vary between which gas the arc is burning in, as well as the current range and which electrode materials that are used. Therefore the potential distribution across a dynamic arc, that burns in air for a typical current range of an LBS is analysed. The analyse are based on results from previously conducted tests performed with the test circuit presented in section \ref{sec:testCir}.

Three interruption test where a thermal re-ignition occurred after the first CZ and the second CZ is selected. The pressure in the pressure chamber was held constant throughout the whole test, and was set to 1.0 bar upstream pressure. The RMS value of the current was 630 A, while the contact and nozzle geometry was the same for each three tests. In figure \ref{fig:threeTests} the arcing voltage during the three chosen interruption tests is shown.
...
\item[7:] Mainly due to elongation of the arc the arcing voltage is higher in this point than in the start (point 1). If the length of the arc where held constant through out the whole experiment the arcing voltage would be expected to be approximately the same at point 7 and point 1. The arc have now burnt a full power cycle, t=0.02 s, from ignition.
\end{description}
\cleardoublepage
