...
For the interruption test two nozzle designs are going to be tested at 630 A and 880 A, and the chance for successfully interrupting the current at a given upstream over-pressure is recorded. One of the nozzle designs are short, narrow and funnel shaped at the end. This nozzle design is never tested before, but are expected to perform well due to the narrow nozzle. For the funnel shaped nozzle design two contact areas are going to be tested. The other nozzle design is longer, wider than the funnel shaped design, it is also cylindrical along the whole nozzle. This design is equal to other previously tested geometries, but the contact area used in this experiment has not been tested before. Previously conducted tests have shown that by increasing the contact and nozzle size, a lower upstream over-pressure is required to interrupt the current. However, above a certain size, increasing the contact and nozzle does not improve the interrupting capability further \cite{AFIMVLBA}. Therefore, it is expected that the cylindrical nozzle design are going to perform almost equal as the other similar nozzle design at 630 A, but at 880 A it will probably perform better compared to the previously tested geometry with a smaller contact area. 
