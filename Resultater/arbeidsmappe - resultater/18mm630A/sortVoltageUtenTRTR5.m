...
%fflush(stdout);
%Calculating the sum
SUM_1=[sum(output_ONE),sum(output_TWO),sum(output_THREE),sum(output_FOUR)];
[m,i]=max(SUM_1);
[n,j]=min(SUM_1);
...
for r=50500:(length(output_ONE)-50500)
	average_Output(r)=(output_ONE(r)+output_TWO(r+(p-l))+output_THREE(r+(z-l))+output_FOUR(r+(x-l)))./4;
end
...
%Standard deviation
Spread=0;
Spread=sqrt(((summer(1)-sumAverage_Output)^2+(summer(2)-sumAverage_Output)^2+(summer(3)-sumAverage_Output)^2+(summer(4)-sumAverage_Output)^2)/3);
